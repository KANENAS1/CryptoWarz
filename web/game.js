...
/* Rides you can buy before you need them. The subway has always sold fares in
   advance, and the game had no answer to the situation everybody in this city
   has been in: money in the bank, nothing in your pocket, wrong side of a
   turnstile. They pair with playing broke - the cheapest way to look poor is
   to actually be carrying nothing. Five rides for the price of four and a
   half, because buying ahead should be worth something. */
const OMNY_RIDES = 5, OMNY_PRICE = 13.00;
...
function Game(seed, tier, perk, gear, difficulty) {
  ...
  this.player = { cash: START_CASH + hard.cash, debt: t.debt * hard.debtMult, vault: 0,
                  cash_cap: this.startingCashCap, vpn: 0, rides: 0, wallet: {} };
...
Object.defineProperty(Game.prototype, "stranded", {
  get() {
    ...
    if ((this.player.rides | 0) > 0) return false;      // a fare already paid for
...
Game.prototype.buyRides = function () {
  this.notNow();
  if (this.player.cash < OMNY_PRICE) throw new Error(`a book of ${OMNY_RIDES} rides is $${OMNY_PRICE.toFixed(2)}`);
  this.player.cash -= OMNY_PRICE;
  this.player.rides = (this.player.rides | 0) + OMNY_RIDES;
  return { text: `${OMNY_RIDES} rides on the card for $${OMNY_PRICE.toFixed(2)}. You have `
    + `${this.player.rides} now, and they do not care how broke you look.`, good: true };
};
...
/* Where you have been and how often, kept where the save can carry it. It is
   deliberately unable to throw: nothing in here is worth losing a day over. */
Game.prototype.noteVisit = function (target) {
  try {
    if (!Array.isArray(this.stats.stations)) this.stats.stations = [];
    ...
    if (!this.stats.visits || typeof this.stats.visits !== "object") this.stats.visits = {};
    this.stats.visits[target.name] = (this.stats.visits[target.name] | 0) + 1;
  } catch (e) { /* a stop you cannot write down is still a stop you made */ }
};
Game.prototype.travel = function (index) {
  ...
  if (this.player.cash + 1e-9 < this.fare) {
    if ((this.player.rides | 0) <= 0) throw new Error(`you can't even make the $${this.fare.toFixed(2)} fare`);
    paidWithCard = true;
  }
  if (paidWithCard) this.player.rides -= 1;
  else this.player.cash -= this.fare;
  /* The station and the day move together, and the bookkeeping comes after.
     They used to be separated by two lines of stats work, and when that work
     threw - which it did, for weeks, on every run restored from the server -
     the ride half happened: the station changed, the day did not, and the
     market never regenerated because that is downstream of the day. From the
     platform: new stop, same prices, same day, forever. */
  this.station = target;
  this.day += 1;
  this.noteVisit(target);
  this.player.debt *= (1 + this.sharkRate);
  this.player.vault *= (1 + VAULT_RATE);
  this.state.drift(this.rng);
  this.market = generate(this.station, this.rng, this.state, undefined, this.luckBySymbol());
...
/* Turning out your pockets, and how fast it stops working as you fill them.
   The only answer whose odds you set yourself, hours earlier, by deciding what
   to carry: empty pockets make it nearly certain, full ones make it a joke. */
const BROKE_BASE = 0.88, BROKE_LOADED = 0.80;
...
function encounterOdds(g, choice) {
  ...
  if (choice === "broke") {
    const cap = Math.max(1, g.player.cash_cap);
    const loaded = Math.min(1, g.player.cash / cap);
    return Math.max(0.05, Math.min(0.95, BROKE_BASE - BROKE_LOADED * loaded));
  }
  if (choice === "relay") {
...
const KINDS = [
  { key: "stickup", title: "SOMEBODY BLOCKS THE STAIRS", severity: 1.0, armable: true,
    options: ["run", "fight", "weapon", "broke", "pay"],
    opening: [
...
function encounterChoices(g) {
  ...
  const built = {
    ...
    broke: { key: "broke", label: "TURN OUT YOUR POCKETS", odds: encounterOdds(g, "broke"),
             note: `Show them the ${n(g.player.cash)} you are carrying and nothing else. `
                   + `They take it and go - if they buy it. The less you carry, the `
                   + `better this works.` },
...
/* Take a fee from the cash, and the shortfall out of the bag.
   ...
   ZERO either way, which made the encounter free exactly when it should have
   hurt, and made the relay strictly worse than paying: a risk taken to save
   nothing. Returns [cash paid, value taken from the bag]. */
function chargeGas(g, owed) {
  const cash = takeCash(g, owed);
  const short = owed - cash;
  if (short <= 0.01) return [cash, 0];
  const held = g.portfolioValue();
  if (held <= 0) return [cash, 0];
  const fraction = Math.min(0.9, short / held);
  confiscate(g, fraction);
...
function paidLine(cash, kind) {
  if (cash > 0 && kind > 0) return `${money2(cash)} in cash, and ${money2(kind)} out of the bag to cover the rest`;
  if (kind > 0) return `${money2(kind)} out of the bag, your pockets being empty`;
  if (cash > 0) return money2(cash);
  return "";
}
...
function gasStandoff(g, choice, was) {
  ...
  const nothing = "You have nothing to pay it with and nothing worth moving. "
    + "The network does not care either way.";

  if (choice === "paygas") {
    const [cash, kind] = chargeGas(g, fee);
    const paid = paidLine(cash, kind);
    out.push(!paid ? nothing
      : `You pay it: ${paid}. Moving your own money, and the block still takes four minutes.`);
    return finish();
  }
  const [cash, kind] = chargeGas(g, fee * RELAY_SHARE);
  const paid = paidLine(cash, kind);
  if (g.rng.random() < encounterOdds(g, "relay")) {
    out.push(!paid ? nothing
      : `The relay works: ${paid}, against ${money2(fee)} through the front door. Nobody asks where it came from.`);
    return finish();
  }
  const lost = confiscate(g, g.rng.uniform(RELAY_TAKES[0], RELAY_TAKES[1]));
  out.push(`The relay was somebody's honeypot. ${paid ? paid + " in fees and " : ""}`
    + `${money2(lost)} of the bag with it. The forum post is gone too.`);
  return finish();
}

/* Convince them there is nothing worth taking. Success costs exactly what is
   in your pockets, which is the point: the one answer whose odds you set
   yourself by deciding how much cash to carry. Failure means they search you,
   and a man who lied about being broke gets treated like one. */
function playBroke(g, kind, out, finish) {
  const carried = g.player.cash;
  if (g.rng.random() < encounterOdds(g, "broke")) {
    const taken = takeCash(g, carried);
    out.push(taken <= 0.01
      ? "You turn out your pockets. Lint, a transfer slip, and a look of genuine disgust "
        + "from a man who has wasted his evening. He goes to find somebody worth robbing."
      : `You turn out your pockets and let him take the ${money2(taken)} that is in them. `
        + `He decides that is all there is, which is the idea.`);
    return finish();
  }
  bumpRep(g, -1);
  const [cash, bag] = rob(g, kind.severity * 1.25);
  out.push(`He does not believe you, and he is right not to. They go through everything: ${lossLine(cash, bag)}`);
  return finish();
}
...
function resolveStandoff(g, choice) {
  ...
  if (choice === "broke") return playBroke(g, kind, out, finish);
...
function saveToDict(g) {
  ...
  return {
    ...
    player: { cash: g.player.cash, debt: g.player.debt, vault: g.player.vault,
              cash_cap: g.player.cash_cap, vpn: g.player.vpn, rides: g.player.rides | 0, wallet },
...
/* A private, mutable copy. The restore used to adopt the caller's objects by
   reference, which was invisible for as long as every save came out of
   JSON.parse - and then the server copy arrived, handed over FROZEN by the
   database, and a whole run became unable to record anything it did. A save
   loader owns its data; it does not borrow somebody else's. */
function thaw(value) {
...
function saveFromDict(data) {
  ...
  if (data.stats) g.stats = thaw(data.stats);
  ...
  g.pending = (data.pending && typeof data.pending === "object") ? thaw(data.pending) : null;
  g.day = data.day;
  ...
  /* and put right what the half-made ride left behind: a run stranded by it is
  ...
  try {
    if (!Array.isArray(g.stats.stations)) g.stats.stations = [];
    if (!g.stats.stations.includes(g.station.name)) g.stats.stations.push(g.station.name);
    if (!g.stats.visits || typeof g.stats.visits !== "object") g.stats.visits = {};
    if (!g.stats.visits[g.station.name]) g.stats.visits[g.station.name] = 1;
  } catch (e) { /* nothing here is worth failing a load over */ }
...
if (typeof module !== "undefined") {
  module.exports = { Game, STATIONS, COINS, COIN, RNG, MarketState, generate, DAYS, SUBWAY_FARE,
                     ...
                     RUN_BASE, FIGHT_BASE, BROKE_BASE, BROKE_LOADED, OMNY_RIDES,
                     OMNY_PRICE, loadPenalty, encounterOdds, KINDS,
                     KIND_BY_KEY, openStandoff, encounterChoices, resolveStandoff,
                     payCost, stickup, bestChoice, TAKE_CASH, lawyerCost, chargeGas,
